...
import { createConversation } from '../api-handlers/conversation';
import { CreateConversationResponse } from '@/types/conversation';
import { ChatBotMessage } from '@/types/message';
import { addMessageToSession } from '../api-handlers/messages';
import { customPersona } from '@/constants/persona';
import UserMessage from './user-message';
import AIMessage from './ai-mesasge';
import { Loading } from '@lemonsqueezy/wedges';

export default function ChatMessage() {
  const [loadingNewMessage, setLoadingNewMessage] = useState(false);
  const [chatInput, setChatInput] = useState('');
  const [messages, setMessages] = useState<(ChatBotMessage | string)[]>([]);
  const [conversation, setConversation] = useState<CreateConversationResponse | null>(null);

  const createNewConversation = async (chatName: string) => {
    const response = await createConversation(chatName);
  ...
  const handleNewMessage = async (message: string) => {
    try {
      if (messages.length === 0) {
        ...
        const session_id = await createNewConversation(message);

        const response = await addMessageToSession(session_id, {
          prompt: message,
          chatbot_model: 'gpt-4',
        ...
        const response = await addMessageToSession(conversation.session_id, {
          prompt: message,
          chatbot_model: 'gpt-4',
          custom_persona: customPersona,
          response_source: 'default',
    ...
    <>
        ...
        <div className='relative h-full'>
          {messages.length === 0 ? (
            ...
            <div className='h-full w-full'>
              <div className='flex flex-col pb-9 text-sm' style={{ overflow: 'auto' }}>
                {messages.map((message, idx) => {
                  if (typeof message === 'string') {
                    return <UserMessage message={message} key={idx} />;
                  } else {
                    return <AIMessage message={message} key={idx} />;
                  }
                })}
              </div>
              {loadingNewMessage ? (
      ...
      <div className='w-full pt-2 md:pt-0 dark:border-white/20 md:border-transparent md:dark:border-transparent md:w-[calc(100%-.5rem)]'>
        ...
        <div className='relative px-2 py-2 text-center text-xs text-token-text-secondary md:px-[60px]'>
          <span>TheoBot can make mistakes. Consider checking important information.</span>
        </div>
      </div>
    </>
  );
